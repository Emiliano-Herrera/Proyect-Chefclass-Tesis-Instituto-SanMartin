...
if ($stmt->execute() === TRUE) {
    ...
    $categorias = isset($_POST['categoria']) ? $_POST['categoria'] : []; // Asegurarse de que esté definida como un array

    // Eliminar duplicados del array de categorías
    $categorias = array_unique($categorias);

    if (!empty($categorias)) {
        // Preparar la consulta para insertar categorías
        ...
        foreach ($categorias as $categoria_id) {
            // Asegurarse de que $categoria_id sea un número entero
            $categoria_id = intval($categoria_id);

            // Vincular los parámetros y ejecutar el statement
        ...
        // Manejar el caso en que no se seleccionaron categorías
    ...
    if (!empty($_FILES['media_files']['name'][0])) {
        foreach ($_FILES['media_files']['name'] as $key => $val) {
            ...
            if (move_uploaded_file($_FILES['media_files']['tmp_name'][$key], $targetFilePath)) {
                ...
                $filePathInDB = "../../uploads/" . $fileName; // Guardar ruta completa con ../../
