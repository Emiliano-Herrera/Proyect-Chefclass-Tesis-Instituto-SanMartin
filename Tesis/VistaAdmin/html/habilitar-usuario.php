...
if (isset($_GET['confirmacion']) && $_GET['confirmacion'] === 'si') {
    ...
    try {
        // Obtener el nombre de usuario antes de habilitarlo
        $stmtSelect = $conexion->prepare("SELECT nombre_usuario FROM usuarios WHERE id_usuario = ?");
        $stmtSelect->bind_param("i", $id);
        $stmtSelect->execute();
        $stmtSelect->bind_result($nombre_usuario_habilitado);
        $stmtSelect->fetch();
        $stmtSelect->close();

        // Habilitar el usuario en la base de datos
        $sql = "UPDATE usuarios SET estado='habilitado' WHERE id_usuario = ?";
        ...
        if ($stmt->affected_rows > 0) {
            ...
            $accion = "Habilitación de usuario";
            $fecha = date('Y-m-d H:i:s'); // Formato para la base de datos
            $detalle = "El usuario '$nombre_usuario_habilitado' ha sido habilitado por '$Nombre $Apellido' en la fecha '" . date('d-m-Y H:i:s') . "'.";
            ...
            $conexion->rollback();
            echo "No se pudo habilitar el usuario.";
        }
