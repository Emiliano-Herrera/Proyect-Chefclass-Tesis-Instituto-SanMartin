...
if ($id) {
    ...
    try {
        ...
        // Habilitar la categoría en la base de datos
        $sql = "UPDATE categoria SET estado = 'habilitado' WHERE id_categoria = ?";
        ...
        if ($stmt->affected_rows > 0) {
            ...
            $accion = "Habilitación de categoría";
            $fecha = date('Y-m-d H:i:s'); // Formato para la base de datos
            $detalle = "La categoría $nombre_categoria ha sido habilitada por $Nombre $Apellido en la fecha " . date('d-m-Y H:i:s') . ".";
            ...
            if ($stmtHistorial->execute()) {
                ...
                $response['message'] = 'La categoría "' . $nombre_categoria . '" ha sido habilitada correctamente.';
            } else {
                $conexion->rollback();
            ...
            $conexion->rollback();
            $response['message'] = 'No se pudo habilitar la categoría.';
        }
