...
if (isset($_SESSION['id_usuario'])) {
    ...
    if (isset($_POST['nombre'])) {
        ...
        try {
            // Obtener el nombre del género antes de editarlo
            $stmtSelect = $conexion->prepare("SELECT nombre_genero FROM generos WHERE ID_Genero = ?");
            $stmtSelect->bind_param("i", $id);
            $stmtSelect->execute();
            $stmtSelect->bind_result($nombre_genero_anterior);
            $stmtSelect->fetch();
            $stmtSelect->close();

            // Actualizar datos del género
            $stmt = $conexion->prepare("UPDATE generos SET nombre_genero = ? WHERE ID_Genero = ?");
            ...
            if ($stmt->affected_rows > 0) {
                ...
                $accion = "Modificación de género";
                $fecha = date('Y-m-d H:i:s'); // Formato para la base de datos
                $detalle = "El género $nombre_genero_anterior ha sido modificado a $nombre por $Nombre $Apellido en la fecha " . date('d-m-Y H:i:s') . ".";
                ...
                if ($stmtHistorial->execute()) {
                    ...
                    $response['message'] = "Has modificado correctamente el género.";
                } else {
                    $conexion->rollback();
                ...
                $conexion->rollback();
                $response['message'] = 'No se pudo editar el género.';
            }
