...
if (isset($_GET['confirmacion']) && $_GET['confirmacion'] === 'si') {
    ...
    try {
        // Obtener el nombre del rol antes de deshabilitarlo
        $stmtSelect = $conexion->prepare("SELECT nombre_rol FROM roles WHERE id_rol = ?");
        $stmtSelect->bind_param("i", $id);
        $stmtSelect->execute();
        $stmtSelect->bind_result($nombre_rol_deshabilitado);
        $stmtSelect->fetch();
        $stmtSelect->close();

        // Deshabilitar el rol en la base de datos
        $sql = "UPDATE roles SET estado='deshabilitado' WHERE id_rol = ?";
        ...
        if ($stmt->affected_rows > 0) {
            ...
            $accion = "Deshabilitación de rol";
            $fecha = date('Y-m-d H:i:s'); // Formato para la base de datos
            $detalle = "El rol '$nombre_rol_deshabilitado' ha sido deshabilitado por '$Nombre $Apellido' en la fecha '" . date('d-m-Y H:i:s') . "'.";
            ...
            $conexion->rollback();
            echo "No se pudo deshabilitar el rol.";
        }
